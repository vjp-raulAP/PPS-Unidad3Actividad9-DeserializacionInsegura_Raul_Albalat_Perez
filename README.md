...
docker-compose up -d
~~~
 Ya tengo inicializado el contenedor de las anteriores actividades por lo que no hace falte que lo vuelva a levantar.
...
La vulnerabilidad se origina por el uso de objetos en la aplicación. Para poder transmitir estos objetos entre diferentes funciones o sistemas a través de la red, es necesario serializarlos, convirtiéndolos en una cadena de texto que pueda ser enviada y posteriormente reconstruida.

El problema aparece cuando los datos serializados proporcionados por el usuario se deserializan sin ningún tipo de validación previa utilizando la función unserialize($_GET['data']). Esta práctica permite a un atacante manipular el contenido del objeto y, por ejemplo, modificar atributos críticos como permisos, otorgándose privilegios no autorizados.

Para ilustrar este comportamiento, crearemos un archivo vulnerable llamado `MostrarObjeto.php`, que contendrá el siguiente código:
...
Además, vamos a crear un archivo con el nombre `GenerarObjeto.php`, que nos permitirá **generar y visualizar los datos serializados de un objeto**, y mostrará un enlace que nos redirige a `MostrarObjeto.php`, pasando dichos datos como parámetro en la URL.

Este archivo es útil para comprender cómo se construye la cadena serializada y cómo se transmite al script vulnerable.
...
Vemos como el objeto serializado que obtenemos al poner de nombre `Raul` es:
...
Cuando es necesario intercambiar objetos entre diferentes módulos o componentes de una aplicación, es común convertir dichos objetos en una cadena utilizando serialización, lo que permite enviarlos a través de la red o almacenarlos fácilmente.

Sin embargo, este mecanismo puede ser aprovechado por un atacante si el código PHP deserializa datos proporcionados por el usuario sin ningún tipo de validación o control. El atacante puede modificar manualmente la cadena serializada y alterar los valores de las propiedades del objeto, como por ejemplo activar privilegios de administrador o ejecutar comportamientos no previstos por la lógica de la aplicación.

Este tipo de vulnerabilidad se clasifica como deserialización insegura y representa una amenaza seria para la seguridad de las aplicaciones web desarrolladas en PHP u otros lenguajes que soporten la serialización de objetos.
...
~~~
MostrarObjeto.php?data=O%3A4%3A%22User%22%3A2%3A%7Bs%3A8%3A%22username%22%3Bs%3A4%3A%22Raul%22%3Bs%3A7%3A%22isAdmin%22%3Bb%3A0%3B%7D
~~~

Podemos cambiar los datos del valor IsAdmin:

~~~
MostrarObjeto.php?data=O%3A4%3A%22User%22%3A2%3A%7Bs%3A8%3A%22username%22%3Bs%3A4%3A%22raul%22%3Bs%3A7%3A%22isAdmin%22%3Bb%3A1%3B%7D
~~~
...
Como nosotros extamos usando docker, o bien entramos dentros del servidor apache y vemos el archivo, o ejecutamos el siguiente comando docker para que nos lo muestre:
